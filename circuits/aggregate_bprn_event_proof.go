...
// AggregateBPrNMsgHCircuit aggregates multiple BPrNMsgHCircuit proofs
...
type AggregateBPrNMsgHCircuit struct {
...
func (c *AggregateBPrNMsgHCircuit) Define(api frontend.API) error {
	...
	// Verify each inner proof and check TargetH consistency
	for i := 0; i < MaxAggregateProofs; i++ {
		...
		// Extract TargetH from inner witness public inputs
		// BPrNMsgHCircuit has TargetH [32]uints.U8 as public input
		...
		// Check TargetH matches (only for active slots)
		for j := 0; j < 4; j++ {
...
func reconstructTargetHFromWitness(api frontend.API, publicInputs []emulated.Element[sw_bls12377.ScalarField]) [4]frontend.Variable {
	...
	// publicInputs[0..31] are the 32 bytes of TargetH
...
// ============================================================================
// AggregateBPrNMsgHCircuitBN254 aggregates multiple BPrNMsgHCircuit proofs
...
// Use AggregateBPrNMsgHCircuit (BLS12-377/BW6-761) for off-chain verification.
//
// Curve: BN254 (inner, emulated) -> BN254 (outer)
type AggregateBPrNMsgHCircuitBN254 struct {
	// Public: common TargetH that all inner proofs share (32 bytes as 4 x 64-bit limbs)
	TargetH [4]frontend.Variable `gnark:",public"`
...
func (c *AggregateBPrNMsgHCircuitBN254) Define(api frontend.API) error {
...
// ============================================================================
// Helper functions for TargetH conversion
// ============================================================================

// TargetHToLimbs converts a 32-byte TargetH to 4 x 64-bit limbs
